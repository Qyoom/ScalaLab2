...
object vector_lab_1 {
	...
	// x + y = 2 // 3 + (-1)
	// z - 1 = 3 // z = 4
	val x = 3; val y = (-1); val z = 4        //> x  : Int = 3
                                                  //| y  : Int = -1
                                                  //| z  : Int = 4
	...
	-u                                        //> Feb 22, 2015 9:29:17 AM com.github.fommil.jni.JniLoader liberalLoad
                                                  //| INFO: successfully loaded /var/folders/qk/q84p77h56y371pyw0vp69j1h0000gn/T/j
                                                  //| niloader7155491571292766522netlib-native_system-osx-x86_64.jnilib
	...
	norm(u3) == eucNorm(u3)                   //> res11: Boolean = true
  ...
  // Normalizing. v_unit is the unique unit vector in the same direction as v
	val v_unit = v ./ (norm(v))               //> v_unit  : breeze.linalg.DenseVector[Double] = DenseVector(0.1348399724926484
                                                  ...
                                                  //| 47794525)
	norm(v_unit)                              //> res12: Double = 1.0
	
	// Cauchy-Schwarz inequality
	norm(u2) * norm(u3)                       //> res13: Double = 38.07886552931954
	u2 dot u3                                 //> res14: Double = 38.0
	abs(u2 dot u3) <= (norm(u2) * norm(u3))   //> res15: Boolean = true
	...
	val u5 = DenseVector(7.3, 2.5, 7.1, 2.9, 8.6, 1, 1.2, 3, 3.4)
                                                  ...
                                                  //| .6, 1.0, 1.2, 3.0, 3.4)
	u4 dot u5                                 //> res16: Double = 144.33999999999997
	norm(u4) * norm(u5)                       //> res17: Double = 243.35933514044618
	abs(u4 dot u5) <= (norm(u4) * norm(u5))   //> res18: Boolean = true
	
	// Triangle inequality or Minkowski inequality
	norm(u4 + u5)                             //> res19: Double = 27.91146717748818
	norm(u4) + norm(u5)                       //> res20: Double = 31.258417590800917
	norm(u4 + u5) <= norm(u4) + norm(u5)      //> res21: Boolean = true
}
/*
